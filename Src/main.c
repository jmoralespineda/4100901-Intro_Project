...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)
{
  if (huart->Instance == USART2)
  {
    ...
    uint8_t transformed_char = rx_data + 1; // Responde siguiente letra del alfabeto
    HAL_UART_Transmit(&huart2, &transformed_char, 1, 10);
...
// Habilitar la recepción UART por interrupción por primera vez
  ...
  char *welcome_msg = "Sistema de Control Basico - Listo, iniciemos\r\n";
  ...
  while (1)
  {
    ...
    if (led_ext_off_time != 0 && HAL_GetTick() >= led_ext_off_time)
    {
      ...
      sprintf(tx_buffer, "¡Sistema activado! LED encendido por 5 segundos.\r\n");
      HAL_UART_Transmit(&huart2, (uint8_t*)tx_buffer, strlen(tx_buffer), 100);
